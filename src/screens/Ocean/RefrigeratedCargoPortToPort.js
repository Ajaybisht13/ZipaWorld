...
import { View, Text, StyleSheet,TextInput, TouchableOpacity, Alert, Button, ScrollView, Modal, ImageBackground , FlatList, ToastAndroid,Platform,AlertIOS, ActivityIndicator,} from 'react-native';
...
class RefrigeratedCargoPortToPort extends Component {
    TotalContainer = [];
  ...
  componentDidMount = async() => {
    this.setState({
      ...
      temperature: this.props.route.params.temperature
    })
    ...
    if(token){
      ...
      this.setState({customerId : authId, customerBranchId : authCustomerBranchId})
    }else {
      this.setState({customerId : guestId, customerBranchId : customerBranchId})
    }
    this.getData();
    this.companyShipper();
 ...
 getBranchData = async() => {
  const token = await AsyncStorage.getItem("userToken");
  const customerBranchId = await AsyncStorage.getItem("customerBranchId");
  fetch(baseUrl + "api/masters/customer/Branch/get", {
    method: "Post",
    headers:{
       "Content-Type" : "application/json",
       "authkey" : token
...
this.setState((previousState) => ({ addConsignee: !previousState.addConsignee }))
  ...
  onChangehandle = (text) => {
    ...
    onSuggestHandle = (text) => {
      ...
      this.setState((previousState) => ({ showCommodity: !previousState.showCommodity }))
      this.setState({commodityHsnCode:text}, () => {
        console.log(text)
      })
    } 
...
getData = () => { 
  fetch(baseUrl + "api/auth/masters/containers/getAll", {
      ...
      body: JSON.stringify({tarrifMode: "Perishable Goods"})
    }).then((response) => response.json())
    .then ((results) => {
      console.log(results);
      this.setState({
...
getQueryRateFunction =(quoteId)=>{
  ...
  .then (async(results) => {
    // console.log("Charges",JSON.stringify(results.result));
    if(results.success == true) {
      this.setState({
        rates: results.result,
...
createRateLoggedUserFunction =async(quoteId, queryCharges)=>{
 ...
 fetch(baseUrl + "api/auth/queries/rates/createRateLoggedUser", {
  ...
  Search = async (queryFor, tarrifMode, activityType, additionalService, temperature) => {
    //alert('here')
    ...
    //console.log("fffffffff",this.state.containerType);
     
    // let selectedContainers = []
    // let totals = 0
    // for(let i=0;i<this.state.containerType.length;i++){
    //   if(this.state.containerType[i]){
    //   if(this.state.containerType[i].count>0){
    ...
    //console.log("aaaaaaaaaaaaaaaaaaaaaaa", JSON.stringify(this.state.ContainerList))

    // console.log("dffffffff", selectedContainers);
    // // let finalPayload = this.state
    // let finalPayload = {
    //   ...this.state,
    //   containerType : selectedContainers
    // }
   // let result = containerType.filter(o => (o.count !== "" && o.count !== 0));
    // console.log("ggggggggggg",result);
    
    // <ActivityIndicator size={"large"}/>
   // console.log('finalPayloadfinalPayloadfinalPayload',JSON.stringify(finalPayload))
...
LCLcontainer = () => {
  ...
  //   console.log("objjjjjjjjjj",containerType[i])
  ...
  //      containerType 
  //   },()=>{
  //     console.log('container-=========>',JSON.stringify(this.state.containerType))
  ...
  //   // console.log('valueeeeeeeeeeeeeeeeeeeeeee',this.state.containerType)
  //   // let total = this.state.containerType.reduce((prevValue, currentValue) => {
  //   //   console.log('valueeeeeeeeeeeeeeeeeeeeeee',prevValue)
  //   //   console.log('valueeeeeeeeeeeeeeeeeeeeeee',currentValue)
  //   //   if(currentValue!== null){
  //   //     if(currentValue.count !== null){
  //   //       return {
  //   //         count : prevValue + currentValue.count
  //   //       }
  //   //     }
  //   //   }
  //   // },0);
  ...
  //     let totals = 0
  //     //console.log('value of capacity-=========>',JSON.stringify(this.state.containerType))
  ...
  //     let totals = 0
  //     //console.log('value of weight-=========>',JSON.stringify(this.state.containerType))
...
volumecapacity = () => {
  //alert(this.state.totalConatinerCbm)
  try {
      if (this.state.volumeWeight !== "") {
          //let grossWeight = parseInt(Math.round(this.state.gross));
          let volumeWeight = parseInt(Math.round(this.state.volumeWeight));
          //let CalculatedGross = grossWeight / 1000;
...
calculateConatainer = () => {
  ...
  this.setState({noOfContainers: sum, totalContainerWeight: sumWeight, totalConatinerCbm: sumCbm,
         ...
         {this.state.getQueryRate ? 
          ...
          <View style = {{display:"flex", flexDirection:"column", padding :"2%",justifyContent:"space-between", marginTop:"10%", height:"100%", paddingBottom:"25%"}}>
           ...
           <TouchableOpacity style={this.state.borderColorId === 1? styles.border : styles.button} onPress={() => this.Select(1)}>
            ...
            <TouchableOpacity style = {styles.btn} onPress = {() => this.Search(queryFor, tarrifMode, activityType, additionalService, this.state.temperature )}>
...
export default RefrigeratedCargoPortToPort;