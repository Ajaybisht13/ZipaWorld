...
class originPort extends Component {
  constructor(props) {
    super(props);
    this.state = {
        record : [],
    ...
    <LinearGradient start={{x: 0.0, y: 0.25}} end={{x: 0.5, y: 1.0}} locations={[0,0.5,0.6]} useAngle= {true} angle={257} colors={['#ffac04', '#fe0000']}
      ...
      <ScrollView style = {{padding:"2%", height:"100%"}}>
        ...
        { this.state.suggestions && this.state.suggestions.map((suggestion , index) =>  
             <TouchableOpacity style = {{padding:"3%", borderWidth:1}} onPress = {() => this.onSuggestHandler(suggestion.hsnCode)}>
                 <Text style = {{fontSize:20}}>{suggestion.hsnCode}</Text>
             </TouchableOpacity>
        )}
         </View>
      </ScrollView>
