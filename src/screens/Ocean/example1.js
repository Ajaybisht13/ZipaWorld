...
<Modal transparent={true} visible={this.state.show5}>
           <View style = {{backgroundColor:"#000000aa", flex:1}}>
              ...
              <Text style = {{fontSize:20, fontWeight:"600"}}>Enter Email</Text>
                  <View style = {{borderWidth:2, borderRadius:10, width:"100%", marginTop:"1%", alignSelf:"center", backgroundColor:"#ffffff", display:"flex", flexDirection:"row", justifyContent:"space-between"}}>
                   ...
                   <Text style = {{fontSize:20, fontWeight:"600"}}>Please Select the Pin code</Text>
                  <Text></Text>
                ...
                { this.state.pinSuggestion && this.state.pinSuggestion.map((suggestion , index) => {
                     return (
                       <View>
                           {
                             suggestion && suggestion.map((item) => {
                              ...
                              return (
                                <TouchableOpacity style = {{padding:"3%"}} onPress = {() => this.onSuggestHandlepin(item)}>
                ...
                <Pressable style = {styles.btn1} onPress={() => this.setState({show5:false})}>
...
class OceanGeneralStep1 extends Component {
  ...
  componentDidMount = async() => {
    ...
    if(token){
      ...
      this.setState({customerId : authId})
    }else {
      this.setState({customerId : guestId})
    }
    this.getData();
    this.companyShipper();
...
showContainerDetails = () => {
  this.setState((previousState) => ({
    shipmentMode:this.state.shipment, 
    show2:false, 
...
showWeightDetail =() => {
  this.setState((previousState) => ({
    show3:false,
    showWeight: !previousState.showWeight,
...
getData = () => { 
  fetch(baseUrl + "api/auth/masters/containers/getAll", {
    ...
    .then ((results) => {
      ...
      this.setState({
        data : results.result,
        containerType: this.state.containerType || results.result
      });
    })
    .catch((error) => console.log("error", error));
}  
...
LCLcontainer = () => {
  ...
  Select = (id) => {
    console.log("container -", this.state.containerType);
  ...
  handleContainerCount = (obj,name, cap, weg ,i) => {
    ...
    console.log("objjjjjjjjjj",obj, containerType, this.state.containerType)
    this.setState({
      containerType,
    },()=>{
    ...
    let total = this.state.containerType.filter(ele => (ele)).reduce((prevValue, currentValue) => {
        return {
    ...
    let TotalCbmCapacity = this.state.TotalContainerCbm.filter(ele => (ele)).reduce((prevCapacity , currentCapacity) => {
      return {
    ...
    let TotalWeightContainer = this.state.TotalContainerWeight.filter(ele => (ele)).reduce((prevWeight , currentWeight) => {
      return {
...
booknow = async(shippingname, noOfContainers, grossWeight, volumeWeight) => {
  ...
  if(token){
    this.setState({
      ...
      this.props.navigation.navigate("getQueryRates", {bookingData: this.state.bookData, grossWeight:this.state.grossWeight, volumeWeight:this.state.volumeWeight, noOfContainers:this.state.containers, bookingCharges:this.state.bookDataCharges, rateId:this.state.id, quoteId:this.state.quoteId, remainingCharges:JSON.stringify(this.state.remainingCharges), revertData:JSON.stringify(this.state.revertLoggedData)})
    })
  }else {
...
submit = async(quoteId, _id, bookData,bookDataCharges, noOfContainers, grossWeight, volumeWeight, containerType, accountType, incoTerms, totalPieces, commodity, chargeableWeight, revertLoggedData, remainingCharges) => {
  ...
  let obj = {
    ...
    customer: {
      "customerId": customerId,
      "customerBranchId": "606ef470e5ab230f74117966",
      "customerName": "Shashank Test Co Pvt Ltd.",
      "customerBranchName": "SHA",
      "pos": "14",
      "gstType": "T-Taxable",
      "gstNo": "09AATCA6549E1Z7",
    },
    "branchId": "600edb3d98380309ac6bd39a",
    "branchName": "Noida-India",
    "branchCode": "01",
    "branchPos": "09",
    "reason": "",
    "buyValue": [],
    "shipperConsigneeData": {
      "shipperName": "AAA 2 Innovate Pvt Ltd",
      "shipperId": "61173948bd436805416ab765",
      "shipperBranchName": "Noida",
      "shipperBranchId": "61173948bd436805416ab766",
      "shipperDetails": "B 29 Third Floor\nSector 1\n Noida Uttar Pradesh India 201301",
      "isStoredShipper": true,
      "consigneeName": "Reliance Mart LLP",
      "consigneeId": "606ef610e5ab230f741179bc",
      "consigneeBranchName": "Catli",
      "consigneeBranchId": "606ef610e5ab230f741179be",
      "consigneeDetails": "Hwang Chau Lane\nRoad No 27\n Catli Masqat Oman 249402",
    ...
    return (
       ...
       <View>
         {this.state.getQueryRate ? 
          ...
          <View style = {{display:"flex", flexDirection:"column", padding :"2%",justifyContent:"space-between", marginTop:"10%", height:"100%", paddingBottom:"25%"}}>
           ...
           <TouchableOpacity style={this.state.borderColorId === 1? styles.border : styles.button} onPress={() => this.Select(1)}>
            ...
            {this.state.showContainers ? 
              ...
              <View>
              <TouchableOpacity style={this.state.borderColorId === 4? styles.border : styles.button} onPress={() => this.Select(4)}>
                <View style = {{display:"flex", flexDirection:"row", width:"100%", justifyContent:"space-between", width:"70%"}}>
                  <Icons name="shipping-fast" size={40} color = "#000000"/>
                  <View style = {{display:"flex", flexDirection:"column"}}>
                  <View style = {{display:"flex", flexDirection:"row"}}>
                      <Text style = {{fontWeight:"700", fontSize:20}}>Shipment Mode :</Text>
                      <Text style = {{fontWeight:"600", fontSize:20, alignSelf:"center"}}> {this.state.shipmentMode}</Text>
                  </View>
                    <View style = {{display:"flex", flexDirection:"row"}}>
                      <Text style = {{fontWeight:"700", fontSize:20}}>No. Of Container :</Text>
                      <Text style = {{fontWeight:"600", fontSize:20 , alignSelf:"center"}}> {this.state.noOfContainers}</Text>
                    </View> 
                    </View> 
                  </View>
                </TouchableOpacity>
          ...
          <View style = {{backgroundColor:"#000000aa", flex:1}}>
          <View style = {{backgroundColor:"#ffffff", marginTop: 100, height:500,padding : 10, borderRadius:10}}>
                        ...
                        return (
                          <View style = {{flexDirection:"row", justifyContent:"space-between"}}>
                              ...
                              <NumericInput rounded 
                                value={this.state.containerType[i].count}
