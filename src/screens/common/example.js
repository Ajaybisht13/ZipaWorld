  ...
  viewProformaInVoice = async() => {
   ...
   hblRequired = () => {
    ...
    companyShipper = async(text) => {
      ...
      //                "authkey" : token
      //           },
      //           body: JSON.stringify({id: consigneeId})
      //           }).then((response) => response.json())
      //           .then((results) => {
      ...
      handleConsignee = async(_id, consigneeName, branchId) => {
        let obj = {}
        obj.consigneeId = _id,
        obj.consigneeName = consigneeName,
          ...
          addHBL =  async() => {
            ...
            .then ((results) => {
              ...
              this.setState({
                ...
                console.log("okkkkkkk", this.state.selectHBL)
                console.log("haaaaawwwwwwlllllbbbbbb", this.state.noOfHawb)
              })
              // hblRequired = () => {
              //   this.setState((previousState) => ({ contents: !previousState.contents }))
              //   }
           ...
           <View style = {{display:"flex"}}>     
            {
                ...
                <Text></Text>  
                 <View>
                    ...
                    <SelectDropdown
                          data={this.state.consigneeNameRecord}
                          onSelect={(selectedItem, index) => {
                            this.setState({
                              _id : selectedItem._id,
                              consigneeName: selectedItem.consigneeName,
                              branchId: selectedItem.branchId._id
                          }, () => {
                              console.log("consigneeee", this.state._id)
                              this.handleConsignee(this.state._id, this.state.consigneeName, this.state.branchId)
                            })
                            console.log(selectedItem, index)
                          }}
                          buttonTextAfterSelection={(selectedItem, index) => {
                            return selectedItem.consigneeName
                          }}
                          rowTextForSelection={(item, index) => {
                            return item.consigneeName
                          }}
                          defaultButtonText = "Select Consignee"
                          buttonStyle ={{width:"100%", borderRadius:10, backgroundColor:"transparent", borderColor:"#000000", borderWidth:2,}}
                        />      
                  <View style = {{marginTop:"3%"}}>
                    <Text style= {{fontSize:15}}>Consignee Branch</Text>
                    {/* <TouchableOpacity onPress = {() => this.conBranch(this.state.consigneeId)}>
                      <Text>Branch</Text>
                    </TouchableOpacity> */}
                    <SelectDropdown
                          ...
                          onSelect={(selectedItem, index) => {
                            console.log(selectedItem, index)
                            this.setState({showConsignee:true})
                          }}
                          buttonTextAfterSelection={(selectedItem, index) => {
                            return selectedItem.branchName
                          }}
